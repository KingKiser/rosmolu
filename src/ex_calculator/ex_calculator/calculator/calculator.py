...
class Calculator(Node):
    ...
    def __init__(self):
        ...
        # Callback group 설정 멀티스레드 콜백 
        self.callback_group = ReentrantCallbackGroup()


        self.declare_parameter('qos_depth', 10) #파라미터 서버 값 불러옴
        ...
        self.arithmetic_argument_subscriber = self.create_subscription(
            ArithmeticArgument,
            'arithmetic_argument',
            self.get_arithmetic_argument, #이게 서브스크라이버의 콜백
            QOS_RKL10V,
        ...
        self.arithmetic_service_server = self.create_service(
            ArithmeticOperator,
            'arithmetic_operator',
            self.get_arithmetic_operator, #서비스의 콜백
            callback_group=self.callback_group)
    ...
    def get_arithmetic_argument(self, msg):
        self.argument_a = msg.argument_a #받은 토픽을 여기에 넣음
    ...
    def get_arithmetic_operator(self, request, response):
        ...
        self.argument_result = self.calculate_given_formula(
            self.argument_a, #토픽 받은 값
            self.argument_b, #토픽 받은 값
            self.argument_operator) #서비스에서 받은 값
        #토픽 계산 값이 변수에 저장
    ...
    def execute_checker(self, goal_handle):
        ...
        #골에 여러 값이 있을 경우 골에서 goal_sum 이 값을 변수에 넣습니다

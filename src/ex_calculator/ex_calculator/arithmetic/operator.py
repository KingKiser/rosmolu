...
from ros_study_msgs.srv import ArithmeticOperator #인터페이스
import rclpy
...
class Operator(Node):
    ...
    def send_request(self): #요청보내는 함수 서비스 서버로 보냄
    ...
    try:
        while rclpy.ok():
            if user_trigger is True:
                ...
                input('Press Enter for next service call.') #input 함수에서 값 받으면 날라가요
                future = operator.send_request()
                user_trigger = True

import random #기본 파이썬 모듈

from ros_study_msgs.msg import ArithmeticArgument #인터페이스
from rcl_interfaces.msg import SetParametersResult #ros 기본 토픽 인터페이스
import rclpy
...
class Argument(Node):
    ...
    def __init__(self):
        ...
        self.arithmetic_argument_publisher = self.create_publisher(
            ArithmeticArgument,
            'arithmetic_argument',#이 이름의 토픽을 발행한다
            QOS_RKL10V)
        self.timer = self.create_timer(1.0, self.publish_random_arithmetic_arguments) #토픽의 주기
        self.get_logger().info('publish topic : arithmetic_argument_publisher')
        self.get_logger().info('end init')

    def publish_random_arithmetic_arguments(self): #콜백함수
    ...
    def update_parameter(self, params): #파라미터에 값 업데이트 반영
